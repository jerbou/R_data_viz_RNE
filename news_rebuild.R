...
setwd("T:/Géobourgogne/DONNEES/PLATEFORME/REFERENTIELS/data_gouv_fr/min_interieur/RNE")
# GET("https://www.data.gouv.fr/fr/datasets/r/67621532-8182-4254-9b55-5e7292f3ab76")

list.files()

# objectif selection les donnees BFC 
# chargement des txt en OD
df_maires <- read.delim("9-rne-maires.txt", sep='\t', header=TRUE, skip=1)
df_cons_mun <- read.delim("1-rne-cm.txt", sep='\t', header=TRUE, skip=1) # conseillers municipaux
df_cons_epci <- read.delim("2-rne-epci.txt", sep='\t', header=TRUE, skip=1) # conseillers communautaire
df_cons_dep <- read.delim("3-rne-cd.txt", sep='\t', header=TRUE, skip=1) # conseillers départementaux
...
View(df_maires)
names(df_maires) <- c("code_dept", "lib_dept", "code_insee", "lib_com", "nom_elu", "prenom_elu", "sexe", "date_naiss", "code_prof", "lib_prof", "date_debut_mandat", "date_debut_fonction")
names(df_cons_mun)
names(df_cons_mun) <- c("code_dept", "lib_dept", "code_insee", "lib_com", "nom_elu", "prenom_elu", "sexe", "date_naiss", "code_prof", "lib_prof", "date_debut_mandat", "lib_fonction","date_debut_fonction", "nationalite")
names(df_cons_epci)
names(df_cons_epci) <- c("code_dept",  "SIREN", "lib_ecpi", "code_insee", "code_dept" ,"lib_com", "nom_elu", "prenom_elu", "sexe", "date_naiss", "code_prof", "lib_prof", "date_debut_mandat", "date_debut_fonction", "nationalite")
dim(df_cons_epci)
names(df_cons_dep) <- c("code_dept", "lib_dept", "nom_elu", "prenom_elu", "sexe", "code_canton", "lib_canton","lib_fonction", "code_prof", "lib_prof","date_naiss", "date_debut_mandat", "date_debut_fonction")

df_marires_bfc <- subset(df_maires, code_dept==21 | code_dept== 25 | code_dept== 39 | code_dept== 58 | code_dept== 70 | code_dept== 71 | code_dept== 89 | code_dept== 90)
dim(df_marires_bfc)
df_cons_mun_bfc <- subset(df_cons_mun, code_dept==21 | code_dept== 25 | code_dept== 39 | code_dept== 58 | code_dept== 70 | code_dept== 71 | code_dept== 89 | code_dept== 90)
dim(df_cons_mun_bfc)

df_cons_epci_bfc <- subset(df_cons_epci, code_dept==21 | code_dept== 25 | code_dept== 39 | code_dept== 58 | code_dept== 70 | code_dept== 71 | code_dept== 89 | code_dept== 90)
dim(df_cons_epci_bfc)
dim(df_cons_epci)

df_cons_dep_bfc <- subset(df_cons_dep, code_dept==21 | code_dept== 25 | code_dept== 39 | code_dept== 58 | code_dept== 70 | code_dept== 71 | code_dept== 89 | code_dept== 90)

df_marires_bfc$fontion_elu <- "maires"
df_cons_mun_bfc$fontion_elu <- "conseiller municipale"
df_cons_epci_bfc$fontion_elu <- "conseiller communautaire"
df_cons_dep_bfc$fontion_elu <- "Conseiller departementaux"

# typeof(df_cons_mun_bfc)
...
# creation de COG
testo$insee_cog <- paste(testo$code_dept, formatC(testo$code_insee, width=3, flag="0"), sep="")
head(testo)
...
testo$fonction_true <- ifelse(testo[13]=="Conseiller departementaux" & is.na(testo$fonction_true), "Conseiller departemental", testo$fonction_true)
testo$fonction_true <- ifelse(testo[13]=="conseiller municipale" & is.na(testo$fonction_true) , "conseiller municipale", testo$fonction_true)
testo$fonction_true <- ifelse(testo[13]=="conseiller communautaire" & is.na(testo$fonction_true) , "conseiller communautaire", testo$fonction_true)
...
dim(testo)
View(testo)
# names(df_cons_mun_bfc)
# head(df_cons_mun_bfc)
...
df_marires_bfc$fontion <- "maires"
df_cons_mun_bfc$fonction <- "conseiller municipale"
df_cons_epci_bfc$fonction <- "conseiller communautaire"
df_cons_dep_bfc$fonction <- "Conseiller departementaux"
